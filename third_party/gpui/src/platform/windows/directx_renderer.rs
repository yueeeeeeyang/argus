...
pub(crate) mod shader_resources {
    ...
    pub(super) fn build_shader_blob(entry: ShaderModule, target: ShaderTarget) -> Result<ID3DBlob> {
        unsafe {
            let (shader_name, shader_source) = if matches!(entry, ShaderModule::EmojiRasterization)
            {
                (
                    b"color_text_raster.hlsl\0".as_slice(),
                    include_str!("color_text_raster.hlsl"),
                )
            } else {
                (b"shaders.hlsl\0".as_slice(), include_str!("shaders.hlsl"))
            };

            // 两份入口 shader 只引用同一个公共文件。交叉构建无法在目标机器保留 Cargo
            // 源码绝对路径，因此在编译期嵌入源码，并在运行时展开唯一的 include 指令。
            let shader_source = shader_source.replacen(
            ...
            let compile_flags = if cfg!(debug_assertions) {
                ...
                // 与 GPUI 原有 release 构建调用 fxc /O3 的优化级别保持一致。
            ...
            let ret = D3DCompile(
                ...
                shader_source.len(),
                PCSTR::from_raw(shader_name.as_ptr()),
                None,
                None,
                entry_point,
                target_cstr,
